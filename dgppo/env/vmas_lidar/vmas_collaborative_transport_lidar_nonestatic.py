...
class VMASCollaborativeTransportLidar(MultiAgentEnv):
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        real_num_agents = jax.random.randint(random_n_agents, shape=(), minval=4, maxval=4)
    ...
    def get_reward(self, graph: GraphsTuple, action: Action) -> Reward:
        ...
        # jax.debug.print("masked_agent_vertex_dists.sum() : {x}", x=masked_agent_vertex_dists.sum() )
    ...
    def get_cost(self, graph: GraphsTuple) -> Cost:
        ...
        if self.params['n_obs'] == 0:
            ...
            mask = jnp.arange(self.num_agents) < env_state.real_num_agents  # shape: (self.num_agents,)
            obs_pos = jnp.where(mask[:, None, None], obs_pos, 1e6)
        ...
        # Reshape obs_pos to (n_points, 2) for the test
        obs_pos_flat = obs_pos.reshape(-1, 2)
        # jax.debug.print("vertex_pos: {x} {y}", x=vertex_pos.shape, y=obs_pos_flat.shape)
        ...
        for i in range(self.num_agents):
            ...
            to_obstacle = obs_pos_flat - v1       # shape (n_objects, top_k_rays, 2)
            ...
            dists = jnp.linalg.norm(obs_pos_flat - closest_point, axis=-1)   # shape (n_objects, top_k_rays)
        ...
        obstacle_object_cost = jnp.full(
            (self.num_agents,),
            jnp.sum(- masked_edge_dists)
        )
        # jax.debug.print("masked_edge_dists: {x}", x=masked_edge_dists)
        # jax.debug.print("obstacle_object_cost: {x}", x=obstacle_object_cost)
        ...
        # jax.debug.print("min_edge_dist: {x}", x=min_edge_dist)
        # jax.debug.print("obstacle_object_cost: {x}", x=obstacle_object_cost)
        # jax.debug.print("final cost: {x}", x=cost)
        return cost
