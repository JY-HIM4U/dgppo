...
class VMASCollaborativeTransportLidar(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.polygon_length= 0.20
        
        
        # self._node_dim = 10  # [x, y, vx, vy] # state dim (6) + indicator: agent: 0001, goal: 0010, object: 0100, obstacle: 1000
        self._node_dim = 18  # agent [x, y, vx, vy] #object [x, y, vx, vy, theta, angvel], relgoal [x,y,theta], state dim (12) + indicator: agent: 0001, lidar: 0010
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        real_num_agents = jax.random.randint(random_n_agents, shape=(), minval=3, maxval=5)
        ...
        # jax.debug.print("real_num_agents: {x}", x=real_num_agents)
        # -------------------------------
    ...
    #     reward -= masked_a_vel.sum() * 0.01
        ...
        # reward -= jnp.where(dist2goal > self.goal_threshold, 1.0, 0.0).mean() * 0.001
        reward -= (jnp.linalg.norm(action, axis=1) ** 2).mean() 
        reward -= masked_agent_vertex_dists.sum() * 10  # * time_factor
    ...
    def get_cost(self, graph: GraphsTuple) -> Cost:
        ...
        for i in range(self.num_agents):
            ...
            v2 = vertex_pos[(i + 1) % self.num_agents]      # shape (2,)
            ...
            mask = jnp.arange(self.num_agents) < env_state.real_num_agents  # shape: (self.num_agents,)
            masked_dists = dists * mask[:, None].astype(dists.dtype)
            
            # For each obstacle, take the minimum distance (across its lidar returns) for this edge
            min_dist_edge = jnp.min(masked_dists, axis=-1)                     # shape (n_objects,)
        ...
        masked_edge_dists = jnp.where(edge_mask[:, None], edge_dists, 1e6)

        # For each obstacle, pick the smallest distance among the valid edges.
        min_edge_dist = jnp.min(masked_edge_dists, axis=0)  # shape (n_objects,)

        # Compute the obstacle cost (here summed over obstacles)
        obstacle_object_cost = jnp.full(
            (self.num_agents,),
            jnp.sum(self.agent_radius - min_edge_dist)
        )
    ...
    def get_graph(self, state: VMASCollaborativeTransportLidarState, lidar_data: Pos2d = None) -> GraphsTuple:
        ...
        node_feats = node_feats.at[0: self.num_agents, 13].set(mask.astype(jnp.float32))
    ...
    def render_video(
        ...
        # jax.debug.print("T_env_states.real_num_agents: {x}", x=T_env_states.real_num_agents)
        # jax.debug.print("goal_theta: {x}", x=goal_theta)
        ...
        base_angles = np.array([i * 2 * np.pi / T_env_states.real_num_agents[0] for i in range(self.num_agents)])
