...
class VMASCollaborativeTransportLidarPolygon(MultiAgentEnv):
    AGENT = 0
    GOAL = 1
    OBS = 2
    OBJECT = 3
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        angles = jnp.array([goal_theta[0] + i * 2 * jnp.pi / self.num_agents for i in range(self.num_agents)])
    ...
    def step(
        ...
        world = World(x_semidim=self.area_size, y_semidim=self.area_size, contact_margin=6e-3, substeps=5, collision_force=500)
    ...
    def render_video(
        ...
        base_angles = np.array([goal_theta + i * 2 * np.pi / self.num_agents for i in range(self.num_agents)])
        ...
        vertex_colors = ["C2", "C1", "C4"]  # Same colors as agent_colors
        ...
        object_vertices = self.object_radius * np.array([
            [np.cos(angle), np.sin(angle)] for angle in base_angles
        ])
        polygon_patch = plt.Polygon(object_vertices, ec="C3", fc="none")
        ...
        def init_fn() -> list[plt.Artist]:
            ...
            return [polygon_patch, *agent_patches, *texts, edge_col]
